...
async function fetchOrDeployGeneric(lens, provider, deploy) {
    ...
    try {
        const deployment = await manifest.lockedRun(async () => {
            debug_1.default('fetching deployment of', lens.description);
            const data = await manifest.read();
            const deployment = lens(data);
            const stored = deployment.get();
            if (stored === undefined) {
                debug_1.default('deployment of', lens.description, 'not found');
            }
            const updated = await deployment_1.resumeOrDeploy(provider, stored, deploy);
            if (updated !== stored) {
...
async function checkForAddressClash(provider, data, updated) {
    ...
    if (clash !== undefined) {
        if (await provider_1.isDevelopmentNetwork(provider)) {
            debug_1.default('deleting a previous deployment at address', updated.address);
            clash.set(undefined);
        }
        else {
